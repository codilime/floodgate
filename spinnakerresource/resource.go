...
func (r Resource) IsChanged() (bool, error) {
	...
	err := r.deleteAutogeneratedFields()
	if err != nil {
		return false, err
	}
...
// deleteAutogeneratedFields is used to delete autogenerated fields from local and remote state
func (r *Resource) deleteAutogeneratedFields() error {
	...
	for _, field := range autogeneratedFields {
		delete(localJSON, field)
		delete(remoteJSON, field)
	}

	localState, err := json.Marshal(localJSON)
	if err != nil {
		return err
	}

	remoteState, err := json.Marshal(remoteJSON)
	if err != nil {
		return err
	}

	r.localState = localState
